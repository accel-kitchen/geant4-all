...
Block::Block(G4String name, G4double pos_X, G4double pos_Y, G4double pos_Z, G4double size_X, G4double size_Y, G4double size_Z, G4int copyNum, G4int nDiv_z, G4String materi, G4PVPlacement*& physVol_World)
{
  ...
  /////// Sensitive Detector
    ...
    auto BlockSV = new SensitiveVolume(std::to_string(sen));
...
//Phantom
Block Phantom1("Phantom1", 0.0 * cm, 0.0 * cm, 20 * cm, 100* mm, 100 * mm, 50 * mm, 0 ,1000, "G4_WATER" ,physVol_World); 
Block Phantom2("Phantom2", 0.0 * cm, 1.0 * cm, 20 * cm, 30* mm, 30 * mm, 30 * mm, 0 ,1000, "G4_Fe" ,physVol_World); 
...
Block Detector("Detector", (i - 3) * 50 * mm, (j - 3) * 50 * mm, 30 * cm, 50* mm, 50 * mm, 10 * mm, i+j*10 , 1, "G4_PLASTIC_SC_VINYLTOLUENE" ,physVol_World); 
  }
}
